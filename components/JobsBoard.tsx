...
/**
 * Metadata for either path the board renders at. The unfiltered board is the
 * home page, so every non-company variant canonicalises to "/"; company views
 * keep the /jobs?company= form that the API docs and existing links use.
 */
export async function jobsBoardMetadata(searchParams: Promise<SearchParams>, deck: string): Promise<Metadata> {
  ...
  return {
    title: { absolute: "Design Jobs | Design Better Careers" },
    description: `${deck} Hundreds of open design roles, pulled daily from company career pages: product design, UX, brand, design systems, research, motion and design leadership.`,
...
export interface JobsBoardProps {
  ...
  /** "/" on the home page, "/jobs" on the jobs page; internal links keep the visitor on the same path. */
  basePath: "/" | "/jobs";
  title: string;
  deck: string;
}

/** The job board. Rendered at "/" (home) and "/jobs"; both are the same list. */
export async function JobsBoard({ searchParams, basePath, title, deck }: JobsBoardProps) {
  const params = await searchParams;
  const [matching, boardTotal, companyRows] = await Promise.all([
  ...
  const heading = params.company ? params.company : title;
        ...
        <div className="flex items-end justify-between mb-10 pb-10" style={{ borderBottom: "1px solid var(--divider)" }}>
          <div>
            ...
            {params.company ? (
              ...
              <p className="text-lg leading-relaxed mt-4 max-w-xl" style={{ color: "var(--text-2)" }}>
                {subheading}
              </p>
            )}
          </div>
